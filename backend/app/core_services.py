from typing import (  # Needed for ProductCreateData, and cast for explicit casting
    NotRequired,
    TypedDict,
    cast,
)
...
from app.models import Integration, Product, Subscription, SyncJob, WebhookEvent
...
class AppService:  # Renamed from CoreService
    ...
    def get_all_products(
        workspace_id: int,
    ) -> list[Product]:  # Moved from ProductService
        stmt: Select[tuple[Product]] = select(Product).filter_by(
    ...
    @staticmethod
    def create_product(
        data: ProductCreateData, workspace_id: int
    ) -> Product:  # Moved from ProductService
        sub: Subscription | None = AppService.get_subscription(
            workspace_id
        )  # Call self
    ...
    def get_integrations(workspace_id: int) -> list[Integration]:
        stmt: Select[tuple[Integration]] = select(Integration).filter_by(
            workspace_id=workspace_id
        )
    ...
    def get_sync_jobs(workspace_id: int) -> list[SyncJob]:
        stmt: Select[tuple[SyncJob]] = select(SyncJob).filter_by(
            workspace_id=workspace_id
        )
    ...
    def get_subscription(workspace_id: int) -> Subscription | None:
        stmt: Select[tuple[Subscription]] = select(Subscription).filter_by(
            workspace_id=workspace_id
        )
    ...
    def get_webhook_events(workspace_id: int) -> list[WebhookEvent]:
        stmt: Select[tuple[WebhookEvent]] = select(WebhookEvent).filter_by(
            workspace_id=workspace_id
        )
