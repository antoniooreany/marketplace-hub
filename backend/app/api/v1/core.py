...
def get_integrations() -> Response:
    return jsonify(content=[{'id': i.id, 'platform': i.platform} for i in CoreService.get_integrations(workspace_id=1)])

@core_bp.route(rule='/sync-jobs', methods=['GET'])
def get_sync_jobs() -> Response:
    return jsonify(content=[{'id': s.id, 'status': s.status} for s in CoreService.get_sync_jobs(workspace_id=1)])

@core_bp.route(rule='/subscription', methods=['GET'])
def get_subscription() -> Response:
    sub: Subscription | None = CoreService.get_subscription(workspace_id=1)
