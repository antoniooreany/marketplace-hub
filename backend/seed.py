...
with app.app_context():
    ...
    for i in range(10):
        p: Product = Product(title=f'Product {i}', sku=f'SKU{i}', price=10.0*i, workspace_id=ws.id)
    ...
    for etype in ['order_created', 'order_updated', 'product_synced', 'error']:
        db.session.add(instance=WebhookEvent(event_type=etype, payload={'data': 'test'}, workspace_id=ws.id))
